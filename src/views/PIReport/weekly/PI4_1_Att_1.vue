...
export default {
	...
	methods: {
		...
		async getList() {
			new Promise((resolve, reject) => {
				...
				let timeStart = moment(this.reportDate).day() == 0 
					? moment(this.reportDate).day(-6).format("YYYY-MM-DD HH:mm:ss") 
					: moment(this.reportDate).day(1).format("YYYY-MM-DD HH:mm:ss");
				if(moment(timeStart).month() != moment(this.reportDate).month()) timeStart = moment(this.reportDate).startOf('month').format("YYYY-MM-DD HH:mm:ss");
				const timeEnd = moment(this.reportDate).startOf('day').add(1, "d").format("YYYY-MM-DD HH:mm:ss");
				timeStart = moment(timeStart).format("YYYY-MM-DD HH:mm:ss");
		...
		storeData(){
			...
			uploadForm.append('pageCount', this.pdfDoc.internal.getNumberOfPages() + 1);
